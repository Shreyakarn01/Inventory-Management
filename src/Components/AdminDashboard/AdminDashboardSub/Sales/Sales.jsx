...
const Sales = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const role = localStorage.getItem('role');
      if (role === 'admin') {
        const response = await fetch(`http://localhost:5000/api/sales/deleteSaleByAdmin/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': token
          }
  ...
  const openModal=async(id)=>{
      const entry = sales.find((sale)=>{return sale._id===id}); //filter cannot be used as it returns an array
      if(entry){
        setFormData({unitPrice: entry.unitPrice,quantitySold : entry.quantitySold,totalPrice:entry.totalPrice});
        setCurrentId(id);
        setIsModalOpen(true);
      }else{
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      if (role === 'admin') {
        const response = await fetch(`http://localhost:5000/api/sales/updateSaleByAdmin/${id}`, {
          method: 'PATCH',
          headers: {
            'Authorization': token,
